...
async function handler(req, res) {
    ...
    if (method === "GET") {
        ...
        return res.status(200).json(gigs.map((g) => ({
            ...g,
            _id: g._id.toString(),
        })));
    }
    ...
    if (method === "PUT") {
        ...
        const r = await col.findOneAndUpdate({ _id: new mongodb_1.ObjectId(id) }, { $set: update }, { returnDocument: "after" });
        if (!r || !r.value)
        ...
        if (calendar) {
            try {
                ...
                if (r && r.value && r.value.calendarEventId) {
                    ...
                    const eventId = calendarRes.data.id;
                    if (eventId) {
                        await col.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { calendarEventId: eventId } });
                    }
                }
            }
            catch (e) {
                console.error(`Error updating/creating calendar event:`, e);
            }
        }
    ...
    if (method === "DELETE") {
        ...
        const gig = await col.findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!gig)
            return res.status(404).json({ error: "Not found" });
        await col.deleteOne({ _id: new mongodb_1.ObjectId(id) });
